  ...
  return (
    <div className="connect-container" style={{ padding: 24 }}>
      ...
      <div style={{background:'#fff', width:'100%', maxWidth:600, borderRadius:12, padding:8, boxShadow:'0 4px 12px rgba(0,0,0,0.15)', maxHeight:'60vh', overflowY:'auto'}}>
        ...
        {chats.map(c => (
          <div key={c.with.id} onClick={()=>onOpenChat(c)} style={{display:'flex', cursor:'pointer', alignItems:'center', padding:'12px 14px', borderBottom:'1px solid #f0f0f0', gap:12}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:13, color:'#555', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:300}}>{c.lastMessage?.text}</div>
            </div>
